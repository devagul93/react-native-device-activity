    ...
    VStack(spacing: 0) {
      ...
      VStack(alignment: .leading, spacing: 0) {
        ...
        HStack(alignment: .bottom, spacing: 0) {
          ...
          if !mostUsedAppsData.isEmpty {
            HStack(spacing: 12) {
              ForEach(Array(mostUsedAppsData.prefix(3).enumerated()), id: \.offset) {
                _, appData in
                AppIconCircle(
...
struct MostUsedAppsReport: DeviceActivityReportScene {
  ...
  func makeConfiguration(representing data: DeviceActivityResults<DeviceActivityData>) async
    ...
    for await dataPoint in data {
      for await activitySegment in dataPoint.activitySegments {
        ...
        let dayKey =
          Calendar.current.dateInterval(of: .day, for: activitySegment.dateInterval.start)?.start
    ...
    let allApps = appUsageMap.map { (_, value) in
      MostUsedAppsData(
    ...
    let insights = generateInsights(
      apps: topApps, hourlyUsage: hourlyUsage, daysInPeriod: daysInPeriod)

    return (topApps, insights)
  }

  private func generateInsights(
    apps: [MostUsedAppsData], hourlyUsage: [Int: TimeInterval], daysInPeriod: Int
  ) -> UsageInsights {
  ...
  private func formatHourRange(start: Int, end: Int) -> String {
    ...
    let startDate =
      Calendar.current.date(bySettingHour: start, minute: 0, second: 0, of: Date()) ?? Date()
    let endDate =
      Calendar.current.date(bySettingHour: end % 24, minute: 0, second: 0, of: Date()) ?? Date()
  ...
  private func generateBehavioralInsight(apps: [MostUsedAppsData], peakHours: String) -> String {
    ...
    let socialMediaApps = apps.filter { app in
      app.categoryName?.lowercased().contains("social") == true
        ...
        || app.appName.lowercased().contains("snapchat") || app.appName.lowercased().contains("x")
    }

    if socialMediaApps.count >= 2 {
      let reductionSuggestion = min(max(20, Int(Double(totalDailyMinutes) * 0.3)), 80)
      return
        "You are most active on social media between \(peakHours). Try blocking apps for \(reductionSuggestion) mins extra"
    } else if totalDailyMinutes > 180 {
      return
        "High daily usage detected between \(peakHours). Breaking sessions into smaller chunks can improve focus"
...
// MARK: - SwiftUI Previews for Fast Iteration
  ...
  struct MostUsedAppsView_Previews: PreviewProvider {
    ...
    static let sampleInsights = UsageInsights(
      ...
      behavioralInsight:
        "You are most active on social media between 9pm-12am. Try blocking apps for 58 mins extra"
    )
